...
* **Level-Triggered Path (Periodic Sweep Fallback):** Independent of Pub/Sub, the controller must execute a full state synchronization sweep every 30s. This guarantees recovery and consistency even if a controller thread crashes, restarts, or misses transient network messages.
...
* **Backoff Rate Limiting:** Consecutive child process crash failures must trigger an exponential wait delay ($5\text{s} \rightarrow 10\text{s} \rightarrow 20\text{s} \dots$) up to 5m to prevent host-resource depletion.
...
* **Level-Triggered Path (Periodic Sweep):** To safeguard against transient connection drops where the subscription might miss an expiration signal, the controller executes a sweep every 30s. It queries the explicit registry of nodes (`nodes:registry`) using non-blocking `SCAN` logic to verify the presence of status keys. Any missing leases trigger eviction.
...
* **Config Sync & Reload Debouncing:** When scaling activities occur in rapid succession (e.g., during a spike scaling event), the Ingress Controller must debounce the reload execution. It buffers dynamic configurations over a 2s window, rewriting the Nginx upstream blocks and executing a single reload command (`nginx -s reload`) to handle the batch without thrashing connections.
...
* **Scale-Up Cooldown:** 30s. The system cannot trigger subsequent scale-up routines while recently created processes are completing cold boot or initialization.
* **Scale-Down Cooldown:** 180s. Prevents rapid container creation/deletion loops ("flapping") when traffic patterns are volatile.
...
Each simulated node is running **Ubuntu 24.04 LTS** inside a system container. By replacing nested Docker with supervised processes running directly in native namespaces, we free up roughly 150MB of idle RAM overhead per node, increasing overall stability.
...
2. Locate and kill the target application process (e.g. Nginx/Go process):
```
...
3. **Expected Result:**
* The `Node Agent` running on `node-1` detects the child process death within 5s via its local process monitor.
...
* **Diagnostic Actions:**
1. Shut down `node-2` from your host shell to simulate a catastrophic hardware failure:
```
incus stop node-2
...
2. Monitor the active Redis state keys using the terminal:
```
...
3. **Expected Result:**
* Within 15s, the Redis key `node:node-2:status` expires. If the keyspace notification is missed, the level-triggered 30-second sweep detects the missing key by scanning `nodes:registry`.
...
* **Diagnostic Actions:**
1. Open the Web Dashboard, navigate to the Deployment panel, and register a deployment with:
* `min_replicas`: 2, `max_replicas`: 6, `target_cpu_percentage`: 50
* `cpu_request`: 150m (0.15 core)


2. On the Load Testing panel, select the deployment and configure an HTTP load that ramps up gradually over time (e.g., $10 \rightarrow 50 \rightarrow 200 \rightarrow 500\text{ RPS}$) to stay within your physical resource bounds.
3. **Expected Result:**
* Telemetry values reported in Redis spike to **90%**.
* Within 15s, the `Autoscaler Engine` computes the scale requirement via the Lua atomic evaluation loop:
